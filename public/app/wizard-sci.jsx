/* eslint-disable */
// Wizard de création SCI (Société Civile Immobilière) — 7 étapes, personne morale, save DRAFT par étape

const STEPS_SCI = ['Société', 'Siège', 'Objet immo', 'Gérant(s)', 'Associés', 'Pièces', 'Récap'];

const NATIONALITES_SCI = [
...
const initSciContent = () => ({
  personneMorale: {
    identite: {
      entreprise: {
        denomination: '', formeJuridique: 'SCI',
    ...
    composition: {
      pouvoirs: [{ individu: emptyGerantSci() }],
      associes: [emptyAssocieSci(), emptyAssocieSci()],
    },
    etablissementPrincipal: {
      ...
      activites: [{ codeApe: '6820A', descriptionDetaillee: 'Location de logements / locaux', dateDebutActivite: '', indicateurPrincipal: true }],
    ...
    _patrimoineDescription: '',
    _apportNatureImmo: false,
  },
});

const WizardSCI = ({ setRoute, dossierId: initialDossierId, onCreated, demoMode = false }) => {
  const W = window.WC;
  const [step, setStep] = React.useState(0);
  const [content, setContent] = React.useState(() => {
    if (demoMode) {
      try {
        const saved = localStorage.getItem('compta_demo_wizard_sci');
        if (saved) return JSON.parse(saved);
      } catch {}
    }
    return initSciContent();
  });
  ...
  const addGerant = () => setContent(c => ({ ...c, personneMorale: { ...c.personneMorale,
    composition: { ...c.personneMorale.composition, pouvoirs: [...c.personneMorale.composition.pouvoirs, { individu: emptyGerantSci() }] } } }));
  ...
  const addAssocie = () => setContent(c => ({ ...c, personneMorale: { ...c.personneMorale,
    composition: { ...c.personneMorale.composition, associes: [...c.personneMorale.composition.associes, emptyAssocieSci()] } } }));
  ...
  const computedClientName = () => entreprise.denomination?.trim() || 'Nouvelle SCI';

  const saveDraft = async () => {
    setSaving(true); setError(null);
    try {
      if (demoMode) {
        try { localStorage.setItem('compta_demo_wizard_sci', JSON.stringify(content)); } catch {}
      } else if (!dossierId) {
        const r = await window.ComptaAPI.apiFetch('/api/dossiers', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ client_name: computedClientName(), type_formalite: 'CREATION', forme_juridique: 'SCI', inpi_content: content }),
        });
        ...
        await window.ComptaAPI.apiFetch(`/api/dossiers/${dossierId}`, {
          ...
          body: JSON.stringify({ client_name: computedClientName(), forme_juridique: 'SCI', inpi_content: content }),
        });
      }
  ...
  return (
    <div style={{ maxWidth: 820 }}>
      <W.ProgressBar steps={STEPS_SCI} current={step} onStepClick={async (i) => { try { await saveDraft(); } catch {} setStep(i); }} />
      ...
      {step === 0 && (
        <W.Section title="La SCI" subtitle="Identité de la société civile immobilière.">
          <W.FieldText label="Dénomination sociale *" value={entreprise.denomination} onChange={v => setEntreprise({ denomination: v })} placeholder="Ex: SCI DU PARC" />
          <W.Row>
          ...
          <W.Row>
            <W.FieldText label="Capital social (€) *" value={String(entreprise.capital)} onChange={v => setEntreprise({ capital: Number((v || '').replace(/[^0-9.]/g, '')) || 0 })} mono placeholder="1000" />
          ...
          <W.FieldSelect label="Régime fiscal" value={fisc.regimeImposition} onChange={v => setFisc({ regimeImposition: v })} options={[
            { value: 'IR', label: 'IR — Transparence fiscale (par défaut SCI)' },
            { value: 'IS', label: 'IS — Option pour l\'impôt sur les sociétés' },
          ]} />
        </W.Section>
      )}
      ...
      {step === 1 && (
        <W.Section title="Siège social" subtitle="Adresse administrative de la SCI.">
          ...
          <W.FieldCheckbox label="Siège chez le gérant (à domicile)" checked={etab.caracteristiques.indicateurExerciceADomicile} onChange={v => setEtab({ caracteristiques: { ...etab.caracteristiques, indicateurExerciceADomicile: v } })} />
        </W.Section>
      )}
            ...
            return (
              <div key={i} style={{ border: '1px solid var(--ink-200)', borderRadius: 10, padding: 14, display: 'grid', gap: 12 }}>
                ...
                <W.Row>
                  ...
                  <W.FieldSelect label="Nationalité" value={g.descriptionPersonne.codeNationalite} onChange={v => updateGerantDesc(i, { codeNationalite: v })} options={NATIONALITES_SCI.map(n => ({ value: n.code, label: n.label }))} />
                </W.Row>
      ...
      <W.Nav step={step} max={STEPS_SCI.length - 1} onPrev={prev} onNext={next} onSubmit={submit} saving={saving} submitting={submitting}
        submitLabel={demoMode ? 'Créer un compte pour soumettre →' : 'Soumettre pour validation'} />
    </div>
  );
};

window.WizardSCI = WizardSCI;
