...
const WizardSASU = ({ setRoute, dossierId: initialDossierId, onCreated, demoMode = false }) => {
  ...
  return (
    <div style={{ maxWidth: 820 }}>
      ...
      {step === 0 && (
        <W.Section title="Votre société (SASU)" subtitle="Vous seul aux commandes : un associé unique, un président. Déjà pré-rempli si vous avez utilisé l'assistant — vérifiez simplement.">
      ...
      {step === 2 && (
        <W.Section title="Votre activité" subtitle="Décrivez ce que fait la société — l'assistant a peut-être déjà tout rempli.">
          ...
          <W.FieldTextarea label="Que fait la société ? *" value={etab.activites[0].descriptionDetaillee} onChange={v => setEtabActivite({ descriptionDetaillee: v })} rows={3} placeholder="Ex: Conseil en stratégie informatique…" />
          <W.Row>
